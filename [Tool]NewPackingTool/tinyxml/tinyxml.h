...
#define MOD_INIMAKER_BY_BLUE

#if !defined(_SERVER)
    #define TINYXML_MOD_BY_BLUE

    #if defined(TINYXML_MOD_BY_BLUE)
...
#if defined(TINYXML_MOD_BY_BLUE)
#if defined( __WIN32__ ) || defined( _WIN32 )
    void SetAttribute( const char* name, const wchar_t * _value )
    {
        int str_num = WideCharToMultiByte( CP_UTF8, 0, _value, -1, NULL, 0, NULL, NULL );
        if ( str_num > 0 )
        {
            char* char_str = new char[str_num];
            WideCharToMultiByte( CP_UTF8, 0, _value, -1, char_str, (int)str_num, NULL, NULL );
            SetAttribute( name, char_str );
            delete [] char_str;
        }
    }
#endif
