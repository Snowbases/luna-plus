...
lb_render_mesh:
		...
		if (m_pGeometry->INL_GetDrawDebugFlag() & DEBUG_DRAW_STATIC_MODEL_COL_MESH)
		{
			...
			//RenderCollisionStaticModel(m_pGeometry->INL_GetDrawDebugFlag());
		}
	}	
	bResult = TRUE;
...
DWORD __stdcall	CoStaticModel::GetMultipleMeshObjectWithRay(PICK_GXOBJECT_DESC* pPickDesc, DWORD dwMaxDescNum, VECTOR3* pv3RayPos,VECTOR3* pv3RayDir,DWORD dwFlag)
{
	DWORD		numPick =0;
	DWORD		i;
	...
	float		fDist;
	VECTOR3		v3IntersectPoint;
	DWORD		dwFaceGroupIndex = 0xffffffff;

	DWORD dwNum = ITGetItemNum(m_pIndexItemTableMeshObject);

	for (i=0; i<dwNum; i++)
	{
		if (IsCollisionMeshObject(&v3IntersectPoint,&fDist,&dwFaceGroupIndex,i,pv3RayPos,pv3RayDir,dwFlag))
		{
			pPickDesc[numPick].gxo = (CMeshObject*)ITGetItemSequential(m_pIndexItemTableMeshObject,i);
			pPickDesc[numPick].fDist = fDist;
			pPickDesc[numPick].v3IntersectPoint = v3IntersectPoint;
...
BOOL __stdcall	CoStaticModel::IsCollisionMeshObject(VECTOR3* pv3IntersectPoint,float* pfDist,DWORD* pdwFaceGroupIndex,DWORD dwObjIndex,VECTOR3* pv3RayPos,VECTOR3* pv3RayDir,DWORD dwFlag)
{
	CMeshObject*	pObj;
	...
	if (IsCollisionMeshAndRay(&v3IntersectPoint,pfDist,pColMeshDesc,pv3RayPos,pv3RayDir))
	{
		if (PICK_TYPE_PER_FACE == dwFlag )
		{
			if (!pObj->IsCollisionMeshAndRay(&v3IntersectPoint,pfDist,pdwFaceGroupIndex,pv3RayPos,pv3RayDir,NULL))
				goto lb_return;
