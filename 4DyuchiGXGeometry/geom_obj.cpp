...
BOOL CMeshObject::IsCollisionMeshAndBoundingBox(VECTOR3* pv3IntersectPoint,float* pfDist,DWORD* pdwFaceGroupIndex,COLLISION_MESH_OBJECT_DESC* pDesc,MATRIX4* pTM)
{
	BOOL		bResult = FALSE;
	BOOL		bNeedDel = FALSE;
	VECTOR3*	pv3Temp = m_pv3World;
	...
	for(i=0; i<24; i++)
	{
		...
		pv3RayFrom = &pDesc->boundingBox.v3Oct[testLine[i].dwVertexIndex[0]];
		pv3RayTo = &pDesc->boundingBox.v3Oct[testLine[i].dwVertexIndex[1]];

		VECTOR3_SUB_VECTOR3(&v3RayDir,pv3RayTo,pv3RayFrom);


		for (DWORD j=0; j<m_dwFaceGroupNum; j++)
		{
			BOOL bRes = m_pFaceGroup[j].IsCollision(&v3IntersectPoint,&fDist,&wIndex,pv3RayFrom,&v3RayDir,pv3Temp);
			if(bRes && (fDist>=0.0f && fDist<=1.0f))
			{
				if (fDist < fMinDist)
				{
					fMinDist = fDist;
