		...
		for( int i = 0; i < SERVERSETMNG->GetMaxServerSet(); ++i )
		{
			...
			for(;iter != pServerSet->m_vecMSServer.end();++iter)
			{
				...
				if( pServer->m_nServerKind == MONITOR_SERVER )
				{
					...
					CString text;
					text.Format( L"%d", pServer->m_nPort );
					m_lcMSInfo.SetItemText( lvitem.iItem, 4, text );
...
void CMSInfoDlg::OnButtonAutopatch() 
{
	...
	if( SERVERSETMNG->IsThereOnServer( str ) )
	{
		_stprintf( temp, L"At First, You Must Off Servers!!" );
		if( MessageBox( temp, L"AutoPatch", MB_OK ) == IDOK )
			return;
	}
	else
	{
		_stprintf( temp, L"Are You Really Execute AutoPatch of %s?", (char*)(LPCTSTR)str );
		if( MessageBox( temp, L"AutoPatch", MB_OKCANCEL ) == IDOK )
		{
			SERVERSETMNG->DoAutoPatch( str );	
			
			LOG( L"Button AutoPatch : %s", (char*)(LPCTSTR)str );
		}
	}
}
