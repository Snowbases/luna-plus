...
{
	CFileDialog dlg( FALSE, NULL, L"*.txt", OFN_FILEMUSTEXIST | OFN_HIDEREADONLY,
					 L"txt 파일|*.txt (*.txt)|*.txt||", NULL );
...
{
	if( m_strCurFileName != L"None" && m_strCurFileName != "" )
	{
		...
		str += L"*MAX ";
		str += m_pNotifyMessage->GetMessage( 0 );
		str += L"\r";	// enter
		str += L"\n";
		for( int i = 1; i <= m_pNotifyMessage->GetMaxLine(); ++i )
		{
			...
			str += ' ';
			str += '"';
			str += m_pNotifyMessage->GetMessage( i );
			str += '"';
			str += L"\r";
			str += L"\n";
		}
...
void CNotifyDlg::SaveFile()
{
	if( m_strCurFileName != L"None" && m_strCurFileName != "" )
	{
		...
		FILE* file = fopen( CW2AEX< MAX_PATH >( m_strCurFileName ), "w+t" );
		if( file )
