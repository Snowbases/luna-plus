...
void CUserCount::InitData()
{
	...
	// treecontrol
	UpdateTreeCtrl();
	SERVERSETMNG->DoQueryChannelInfo( L"All Server" );
}

HTREEITEM CUserCount::InsertItem( HTREEITEM hRoot, LPTSTR name, BOOL bChild )
{
...
void CUserCount::SetChannelData( int nServerSet, MSG_CHANNEL_INFO_MORNITOR* pChannelInfo )
{
	...
	if( pServerSet )
	{
		CServer* pServer = pServerSet->GetServer( 
			CA2WEX< sizeof( pChannelInfo->sServerIP ) >( pChannelInfo->sServerIP ),
			pChannelInfo->wServerPort );
...
void CUserCount::UpdateTreeCtrl()
{
				...
				for(;iter != pServerSet->m_vecServer.end();++iter)
				{
					...
					if( pServer->m_nServerKind == MAP_SERVER )
					{
						if( pServer->m_nState == eServerState_Off )
							_stprintf( temp, L"%s : 0", pServer->m_sName );							
						else
							_stprintf( temp, L"%s : %d", pServer->m_sName, pServer->m_dwUserCount );
						...
						else
						{
							...
							{
								_stprintf( temp, L"Channel %d : %4d", j+1, pServer->m_nChannelUserCount[j] );
	...
	else
	{
		...
		pServerSet = SERVERSETMNG->GetServerSet( str );
		if( !pServerSet )	
			return;
