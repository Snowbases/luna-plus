...
#pragma pack(push,1)
// 090511 LUJ, 툴 버전을 명시함. 이전에는 특정 버전 값을 넘기면 계정 검사를 하지 않았음
#define SERVERTOOLVERSION "LUNA07062101"
...
enum eTOTALLOGKIND
{
	...
	eTotalLogKind_Skill,
	eTotalLogKind_SkillExp,
	eTotalLogKind_Money,
	eTotalLogKind_MoneyWrong,	
	/*어빌리티 삭제 - eTotalLogKind_Ability,*/
...
struct PHYOKUK_TOTALINFO
{
	WORD		PyokukCount;
	DWORD		Money;
	ITEMBASE	Pyokuk[SLOT_STORAGE_NUM];
};
...
struct TMSG_CHARACTER_STORAGEINFO : public TMSGBASE
{
	char		m_sUserName[17];
	DWORD		m_dwUserIdx;
	WORD		m_wStorageNum;
...
struct TMSG_SKILLLOG : public TMSGBASE
{
	BOOL		bEnd;
	WORD		wCount;
	DWORD		dwCharIdx;
...
struct TMSG_SKILLEXPLOG : public TMSGBASE
{
	BOOL		bEnd;
	WORD		wCount;
	DWORD		dwCharIdx;
...
{
	DWORD				dwCount;
	ITEM_OPTION	ItemOptionInfo[100];

	TMSG_ITEMOPTIONINFO2()
	{
		dwCount = 0;
		memset( ItemOptionInfo, 0, sizeof(ITEM_OPTION)*100 );
	}
	void	Add( ITEM_OPTION* pInfo )
	{
		ItemOptionInfo[dwCount] = *pInfo;
		++dwCount;
	}
	int		GetSize()	{ return sizeof(TMSG_ITEMOPTIONINFO2) - sizeof(ITEM_OPTION)*(100-dwCount); }
};
