...
void CServerTable::Release()
{
	if( m_pSelfServerInfo)
	{
		...
		// 같은 코드가 DistributeServer와 AgentServer, MapServer 모두에 복사본으로 존재한다.
...
void CServerTable::AddServer(SERVERINFO * info, WORD Port)
{
	...
	// 081106 NYJ - 리스트에 목록이 존재하면 갱신, 없으면 삽입.
	SERVERINFO* pServerInfo = CYHHashTable<SERVERINFO>::GetData(Port);
	if(pServerInfo && pServerInfo->dwConnectionIndex==0)
	{
		memcpy(pServerInfo, info, sizeof(SERVERINFO));
	}
	else
	{
		CYHHashTable<SERVERINFO>::Add(info, Port);
...
void CServerTable::RemoveServerForKind( WORD wSrvKind )
{
	SetPositionHead();
	SERVERINFO* pSrvInfo = NULL;
	while( (pSrvInfo = (SERVERINFO*)GetData()) != NULL)
	{
		if(pSrvInfo->wServerKind == wSrvKind)
		{
			Remove(pSrvInfo->wPortForServer);
			delete pSrvInfo;
		}
	}
}
