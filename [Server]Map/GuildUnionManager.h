...
public:
	...
	void	SendChatMsg( MSG_GUILDUNION_CHAT* pMsg );				// 090317 ONS 에이전트서버의 패킷조작 방지처리에 따른 메세지 변경(MSG_CHAT_WITH_SENDERID -> MSG_GUILDUNION_CHAT)

	void	AddPlayer( CPlayer* pPlayer, DWORD dwGuildUnionIdx );
	void	LoginPlayer( CPlayer* pPlayer, DWORD dwGuildUnionIdx );
	void	SetInfoToPlayer( CPlayer* pPlayer, DWORD dwGuildIdx );
	...
	CGuildUnion* GetUnion( DWORD unionIndex );
};
