...
void CObjectProperty::SetGXObjectProperty(GXOBJECT_PROPERTY* pProperty, GXOBJECT_HANDLE handle)
{
...
void CObjectProperty::OnButtonOk() 
{
	...
	CMoveObjectHistory& history = g_pTool->GetHistoryManager().Allocate< CMoveObjectHistory >();
	history.SetValue(
		*m_pObjectProperty,
		mHandle);
...
BOOL CObjectProperty::OnInitDialog() 
{
	...
	m_strName = m_pObjectProperty->name;
