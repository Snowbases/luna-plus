...
BOOL CDialogTilePalette::Initialize(DWORD dwTileSize,CDialogEditEntry* pEditEntryDlg, CDialogTileSet* pTileSetDlg)
{
	...
	HMODULE handleModule = LoadLibrary("Renderer.dll");
	...
	const CREATE_INSTANCE_FUNC function = (CREATE_INSTANCE_FUNC)GetProcAddress(
		handleModule,
		"DllCreateInstance");
	const HRESULT handleResult = function((LPVOID*)&m_pRenderer);
...
void CDialogTilePalette::OnButtonReplaceTexture() 
{
	CFileDialog dialog(
		...
		_T( "Targa Files(*.tga)|*.tga|Tiff Files(*.tif) |*.tif||" ),
		0);	
	dialog.DoModal();
	...
	// 작업 취소 처리를 한다
	{
		...
		HFIELD_DESC fieldDesc = {0};
		heightField->GetHFieldDesc(&fieldDesc);
		const CString oldTilePath(fieldDesc.pTexTable[m_wSelectedTileIndex].szTextureName);

		CHistoryManager& historyManager = g_pTool->GetHistoryManager();
		CReplaceTileHistory& history = historyManager.Allocate< CReplaceTileHistory >();
		history.SetValue(
			m_wSelectedTileIndex,
			oldTilePath,
			dialog.GetPathName());
	}
...
void CDialogTilePalette::UpdateTile(LPCTSTR path, DWORD index)
{
	IDISpriteObject* const sprite = m_pRenderer->CreateSpriteObject(
		LPTSTR(path),
		SPRITE_CREATE_IMAGE);
	...
	IDISpriteObject*& oldSprite = m_ppSpriteTable[index];
...
{	
	LPCTSTR szTilePalleteName = g_pTool->GetCurrentTilePalleteFileName();
	char szActualFileName[_MAX_PATH] = {0};
	...
	char szCaption[_MAX_PATH + 128] = {0};
