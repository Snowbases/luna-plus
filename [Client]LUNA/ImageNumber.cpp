...
// desc_hseos_몬스터미터01
// S 몬스터미터 추가 added by hseos 2007.05.29
void CImageNumber::InitDamageNumImage(cImageSelf* pImg, int nNum)
{
	m_pImg[nNum] = pImg;
}
// E 몬스터미터 추가 added by hseos 2007.05.29
...
// desc_hseos_몬스터미터01
// S 몬스터미터 추가 added by hseos 2007.05.29
void CImageNumber::RenderWithDamageNumImage()
{
////ÀÓ½Ã ¾Ö´Ï
	DWORD dwAlpha = 255;
	if( m_bFadeOut )
...
//
	...
	for( DWORD i = 0 ; i < m_dwLimitCipher ; ++i )
	{
		nChar = nNum % 10;
		nNum /= 10;
		m_pImg[nChar]->RenderSprite( &m_vScale, NULL, 0.0f, &vPos, 0x00ffffff | (dwAlpha<<24)  );
