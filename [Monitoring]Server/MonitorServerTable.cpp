...
CMonitorServerTable::CMonitorServerTable()
{
	...
	m_pMSInfo = NULL;
	// 080118 KTH --
	m_ServerInfoTable.Initialize( 50 );
...
void CMonitorServerTable::LoadMonitorInfo(WORD typeServer,char* pMyPublicIp,char* pMyPrivateIp)
{
	...
	{	
		...
		SERVERLISTMGR->SetPositionHead();
 		...
 		{
 			if( strcmp(pInfo->szIPForServer, m_pMSInfo->szIPForServer) == 0 )
			if( pInfo->wServerKind != MONITOR_SERVER )
 			if( pInfo->wServerKind != MONITOR_AGENT_SERVER )
 			{
					MSERVERINFO* pMInfo = m_InfoPool.Alloc();
					memset(pMInfo, 0, sizeof(MSERVERINFO));
					strcpy(pMInfo->szIPForServer, pInfo->szIPForServer);
					pMInfo->wPortForServer = pInfo->wPortForServer;
 					strcpy(pMInfo->szIPForUser, pInfo->szIPForUser);
 					pMInfo->wPortForUser = pInfo->wPortForUser;
					pMInfo->wServerType = pInfo->wServerKind;

 				m_ServerInfoTable.Add( pMInfo, pMInfo->wPortForServer );
 			}
 		}
	}
}
