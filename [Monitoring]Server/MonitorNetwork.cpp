...
BOOL CMonitorNetwork::Init(DESC_NETWORK * desc)
{
	...
	if (FAILED(hr))
		return FALSE;
	// 080813 LUJ, 수정된 inetwork.dll이 적용되도록 수정
...
void CMonitorNetwork::DisconnectUser(DWORD dwConnectionIdx )
{
	m_pINet->CompulsiveDisconnectUser( dwConnectionIdx );
}

void CMonitorNetwork::ConnectToServers()
{
	MSERVERINFO*	pInfo	=	NULL;
	MServerTable->m_ServerInfoTable.SetPositionHead();

	while( pInfo = MServerTable->m_ServerInfoTable.GetData() )
	{
		m_pINet->ConnectToServerWithServerSide( pInfo->szIPForServer, pInfo->wPortForServer, CMSSystem::OnConnectServerSuccess, CMSSystem::OnConnectServerFail,pInfo);
	}
}