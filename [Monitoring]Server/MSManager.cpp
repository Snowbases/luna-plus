...
void CMSManager::SendServerState(WORD wState, MSERVERINFO* pInfo)
{
	...
	// 090709 --- ServerTool에서 Off 실행여부
	msg.bOffByServerTool			= pInfo->bOffByServerTool;
...
void CMSManager::AddServerRegiest(DWORD dwConnectionIdx, DWORD ProcessID, SERVERINFO * pInfo)
{
	...
	__time64_t tick;
	_time64( &tick );
	Info.ExpiredTime = tick;
	Info.wServerState = SERVERSTATE_ON;
	// 090709 --- ServerTool로부터 Off정보 여부 추가
	Info.bOffByServerTool = FALSE;
...
BOOL CMSManager::ServerOff(WORD Port)
{
	...
	if( pInfo )
	{
		// 090709 --- ServerTool로부터 Off정보 설정
		pInfo->bOffByServerTool = TRUE;
...
BOOL CMSManager::KillServer(WORD Port)
{
	...
	if( pInfo )
	{
		...
		hProcess = OpenProcess(PROCESS_ALL_ACCESS, TRUE, pInfo->ProcessID);
