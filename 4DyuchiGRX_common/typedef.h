...
// 090210 LYW, 090427 ShinJS --- typedef : Attach 시키는 개수를 늘인다 (탈것 최대 20인승에 의해 최대 20으로 수정)
#define MAX_ATTATCH_OBJECTS_NUM			20
...
// @@@
// Iros Render 할 객체들을 옵션으로 나누자.
// 기존에 DEFAULT는 그대로 사용.
enum EXECUTIVE_RENDER_MODE
{
	RENDER_MODE_OBJECT = 0x00000001,// 오브젝트를 세분화 할수 있도록 하자.

	RENDER_MODE_HFIELD = 0x00000010,// 맵에 관련된 데이타를 세분화 할 자리.

	RENDER_MODE_TOOL   = 0x10000000,// Tool 에서만 사용되는 Object
};

#define RENDER_MODE_DEFAULT		RENDER_MODE_OBJECT|RENDER_MODE_HFIELD
...
struct DISPLAY_INFO
{
	...
	DWORD			dwRefreshRate;
	DWORD			dwBPS;
	DISPLAY_INFO&	operator=(const DISPLAY_INFO &info)
	{
		this->dispType		= info.dispType;
		this->dwWidth		= info.dwWidth;
		this->dwHeight		= info.dwHeight;
		this->dwRefreshRate	= info.dwRefreshRate;
		this->dwBPS			= info.dwBPS;
