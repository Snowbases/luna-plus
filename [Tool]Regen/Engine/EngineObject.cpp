...
void CEngineObject::LoadInfoFile(char* filename)
{
	...
	for(WORD MotionNum = file.GetWord();
		0 < MotionNum;
		MotionNum = file.GetWord())
	{
...
{
	if(pWeaponObject->Attach(this,szObjectName))
	{
		mEngineObjectContainer.push_back(
			pWeaponObject);
	}
...
{
	while(false == mEngineObjectContainer.empty())
	{
		SAFE_DELETE(*(mEngineObjectContainer.begin()));
		mEngineObjectContainer.pop_front();
	}
}
