...
void ProcessGarbageObject()
{
	...
	for(GXOBJECT_HANDLE handle = (GXOBJECT_HANDLE)GarbageObjectList.RemoveTail();
		0 < handle;
		handle = (GXOBJECT_HANDLE)GarbageObjectList.RemoveTail())
	{
...
void ReleasePool()
{
	for(EffectGarbage* garbage = (EffectGarbage*)EffectPool.RemoveTail();
		0 < garbage;
		garbage = (EffectGarbage*)EffectPool.RemoveTail())
	{
		SAFE_DELETE(
			garbage);
	}
