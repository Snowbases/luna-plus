...
//	LISTCONDITION* pListCondition = pDoc->GetConditionList();
	for( itc = pDoc->GetConditionList().begin(); itc !=pDoc->GetConditionList().end(); ++itc)
	{
		...
		{
			sztemp.Format("\t#ADDCONDITION\t%s\t%s\t%s\t%s", pGroupCondition->AddCondition[i].szTargetGroupID, 
				pGroupCondition->AddCondition[i].szCondition, pGroupCondition->AddCondition[i].szRegenTime,"1");
			m_MonsterListBox.AddString(sztemp);
			SelCount++;
