...
BOOL CFontCache::GetResource(LPDIRECT3DTEXTURE9* ppTex,CVertexBuffer** ppVB,CIndexBuffer** ppIB,RECT* pRect,FONT_PROPERTY_DESC* pProperty)
{
...
*/
	...
	pBlockDesc->wVBWidth = (WORD)(pRect->right - pRect->left);
	pBlockDesc->wVBHeight = (WORD)(pRect->bottom - pRect->top);
...
lb_set_vb:
	...
	m_v4Rect[0].x = (float)pRect->left;
	m_v4Rect[0].y = (float)pRect->top;
	
	m_v4Rect[1].x = (float)(pRect->left + pBlockDesc->wVBWidth - 1);
	m_v4Rect[1].y = (float)pRect->top;
	
	m_v4Rect[2].x = (float)pRect->left;
	m_v4Rect[2].y = (float)(pRect->top + pBlockDesc->wVBHeight - 1);
	
	m_v4Rect[3].x = (float)(pRect->left + pBlockDesc->wVBWidth - 1);
	m_v4Rect[3].y = (float)(pRect->top + pBlockDesc->wVBHeight - 1);
...
lb_find_ok:
	...
	pBlockDesc->wWidth = (WORD)dwWidth;
...
//	TEXTURE_BUFFER_DESC*	pBackBufferDesc;
	...
	pLineDesc->wAvailableWidth = (WORD)m_dwTexWidth;
	pLineDesc->wCurrentPosX = 0;
	pLineDesc->wHeight = (WORD)dwHeight;
