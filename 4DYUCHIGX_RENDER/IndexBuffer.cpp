...
BOOL CIndexBuffer::CreateIndexBuffer(CD3DResourceManager* pResourceManager,WORD* pIndex,DWORD dwUsage,D3DPOOL pool,DWORD dwNum)
{
	...
	if (pIndex)
	{
		if(D3D_OK != m_pIB->Lock(0, 0, (void**)&pDest, 0))
		{
			goto lb_return;
		}
...
BOOL CIndexBuffer::SetIndexList(WORD* pIndex,DWORD dwNum)
{
	...
	if(D3D_OK != m_pIB->Lock(m_dwStartIndex*sizeof(WORD),sizeof(WORD)*m_dwIndicesNum,(void**)&pDest, 0 ))
	{
		goto lb_return;
	}
...
BOOL CIndexBuffer::LockPtr(WORD** ppWord)
{
	...
	if(D3D_OK != m_pIB->Lock(m_dwStartIndex*sizeof(WORD),sizeof(WORD)*m_dwIndicesNum,(void**)ppWord, 0 ))
	{
		*ppWord = NULL;
		goto lb_return;
	}
