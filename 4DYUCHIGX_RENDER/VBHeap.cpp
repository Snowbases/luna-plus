...
BOOL __stdcall CVBHeap::AllocVB(DWORD* pdwErrorCode,CVertexBuffer* pVertexBuffer,DWORD dwVerticesNum,DWORD* pdwSize,BOOL* pbEnable,IResourceController* pResourceController)
{
	...
	bResult = m_D3DRCBufferHeap.AllocBuffer(pdwErrorCode,(IDirect3DResource9**)&pVertexBuffer->m_pVB,&pVertexBuffer->m_pVBHandle,&pVertexBuffer->m_dwStartIndex,NULL,dwVerticesNum,pbEnable,pResourceController);
	if (!bResult)
		goto lb_return;
