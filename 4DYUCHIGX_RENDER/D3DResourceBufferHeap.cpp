...
BOOL CD3DResourceBufferHeap::Initialize(CD3DResourceManager* pResourceManager,DWORD dwFVF,DWORD dwUsage,D3DPOOL pool,DWORD dwUnitSize,DWORD dwMaxUnitsNumPerBuffer,DWORD dwMaxBufferNum,DWORD dwMaxSize,OnAllocBufferFunc pAllocBufferFunc,DWORD dwFlag)
{
...
#ifdef _DEBUG
	VHPInitialize(m_pHeap,m_pMassMemory,m_dwMassMemorySize,8192*4,m_dwUnitSize,m_dwUnitNumPerD3DRCBufferPool,TRUE);
#else
	VHPInitialize(m_pHeap,m_pMassMemory,m_dwMassMemorySize,8192*4,m_dwUnitSize,m_dwUnitNumPerD3DRCBufferPool,FALSE);
#endif
	...
	InitializeStaticMemoryPool(m_pHeapItemDescPool,sizeof(D3DRC_HEAP_ITEM),8192,8192*4);
