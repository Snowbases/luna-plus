		...
		)
{
	...
	LPDIRECT3DTEXTURE9	pLightTex;
	LIGHT_DESC*			pLight;
	
	LPDIRECT3DDEVICE9	pDevice = m_pRenderer->GetDevice();
	CAMERA_DESC*		pCamera;
	MATRIX4*			pResult = m_pRenderer->GetIdentityMatrix();
	LPDIRECT3DTEXTURE9	pDefaultTex = m_pRenderer->GetDefaultMaterial()->TextureDiffuse.GetTexture();
...
lb_render:
	...
	pDevice->SetFVF(m_VB.GetVertexShader());
	
	pDevice->SetStreamSource( 0,m_VB.GetVertexBuffer(),0,m_VB.GetVertexSize());
	...
	//	pMtl = m_pRenderer->GetMaterial(1,i);
		...
		pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,m_VB.GetStartIndex(),0,m_dwVertexNum,m_pFaceGroup[i].GetIBStartIndex(),m_pFaceGroup[i].GetFacesNum());
...
//	m_pRenderer->SetRenderMode(0);
	...
	pDevice->SetVertexShader(NULL);
	pDevice->SetFVF(m_VBProjected.GetVertexShader());
	pDevice->SetStreamSource( 0,m_VBProjected.GetVertexBuffer(),0,m_VBProjected.GetVertexSize());
...
//				pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,m_dwVertexNum,m_pFaceGroup[i].GetIBProjectedStartIndex(),m_pFaceGroup[i].GetCulledWithLightFacesNum());
				...
				pDevice->SetIndices(m_pFaceGroup[i].GetIBPrjLightPtr(j)->GetIndexBuffer());
				pDevice->DrawIndexedPrimitive(
					D3DPT_TRIANGLELIST,
					m_VBProjected.GetStartIndex(),
					0,
					m_dwVertexNum,
	...
	if (bShadow)
	{
		...
		for (DWORD m=0; m<dwSpotLightNum; m++)
		{
			...
			for (i=0; i<m_dwFaceGroupNum; i++)
			{
				...
				pDevice->SetIndices(m_pFaceGroup[i].GetIBPrjShadowPtr(m)->GetIndexBuffer());
				pDevice->DrawIndexedPrimitive(
					D3DPT_TRIANGLELIST,
					m_VBProjected.GetStartIndex(),
					0,
