...
*/
/*
void DecompressDXT1(char* pBits,IDirect3DSurface9* pSurface)
{
...
BOOL CD3DResourceManager::CreateTextureWithDDS(IDirect3DTexture9** ppTexture,char* szFileName)
{
...
//	ddsd.dwFlags &= (~DDSD_LINEARSIZE);
	...
	if (!bUseCompress)
	{
		...
		IDirect3DTexture9*	pTexResult;
		IDirect3DSurface9*	pSurfaceResult;
...
BOOL CD3DResourceManager::Initialize(CoD3DDevice* pRenderer,IDirect3DDevice9* pDevice,DWORD dwMaxItemNum)
{
								...
								)
{
	...
	hr = m_pD3DDevice->CreateVertexBuffer(Length,Usage,FVF,Pool,ppVertexBuffer,NULL);
								...
								)
{
	...
	hr = m_pD3DDevice->CreateIndexBuffer(Length,Usage,Format,Pool,ppIndexBuffer,NULL);
								...
								)
{
	...
	// @@@ Iros D3DX 함수를 활용하자.
	hr = D3DXCreateTexture(m_pD3DDevice, Width,Height,Levels,Usage,Format,Pool,ppTexture );
	//hr = m_pD3DDevice->CreateTexture(Width,Height,Levels,Usage,Format,Pool,ppTexture,NULL);
	if (hr != D3D_OK)
	{
...
HRESULT CD3DResourceManager::CreateFont( CONST D3DXFONT_DESC* pFontDesc, LPD3DXFONT* ppFont )
{
	D3DRESOURCE_DESC* pDesc = (D3DRESOURCE_DESC*)LALAlloc(m_pRCDescPool);
	...
	///※ 폰트형이 dx9설명서에 있는것과 다름.. 캐스팅
	HRESULT hr = D3DXCreateFontIndirect(m_pD3DDevice, pFontDesc, ppFont);
	if (hr != D3D_OK)
	{
		LALFree(m_pRCDescPool,pDesc);
	...
	pDesc->pResource = *(IDirect3DResource9**)ppFont;			// 이 부분은 예외처리를 해야한다.FONT는 따로 처리.
...
void CD3DResourceManager::OnLostFont()
{
	...
	for (DWORD i=0; i<dwNum; i++)
	{
		if (ppRestoreItem[i]->type == D3DRESOURCE_TYPE_D3DXFONT)
		{
			...
			HRESULT hr = pFont->OnLostDevice();
			if (hr != D3D_OK)
			{
				OutputDebugString("pFont->OnLostDevice() Failed!!\n");
			}
		}
	}
...
BOOL CD3DResourceManager::CreateTextureWithBitmap(LPDIRECT3DTEXTURE9* ppTex,DWORD dwUsage,D3DFORMAT format,D3DPOOL pool,char* pBits,DWORD dwWidth,DWORD dwHeight,BOOL bUseMipmap,BOOL bCompress)
{
	BOOL		bResult = FALSE;
	HRESULT		hr;
...
/*
BOOL CD3DResourceManager::CreateBumpMap(LPDIRECT3DTEXTURE9* ppBumpMap,LPDIRECT3DTEXTURE9 pSrc)
{
...
//	IDirect3DSurface9*	pSurfaceDest;
	...
	for (i=0; i<dwLevelCount; i++)
	{
		...
		IDirect3DSurface9*	pSurfaceTemp;
		pSrc->GetSurfaceLevel(i,&pSurfaceTemp);

		IDirect3DSurface9*	pBumpSrc;
...
BOOL CD3DResourceManager::SetSurfaceWithBitmap(DWORD dwSurfIndex,LPDIRECT3DTEXTURE9 pTex,char* pBits,DWORD dwWidth,DWORD dwHeight,D3DFORMAT format)
{
	BOOL			bResult = FALSE;
	HRESULT			hr;
	...
	if (IsCompressed(ddsc.Format))
		m_pD3DDevice->CreateOffscreenPlainSurface(ddsc.Width,ddsc.Height,format,D3DPOOL_SCRATCH,&pSurfaceTemp,NULL);
	else
...
BOOL CD3DResourceManager::CreateBitmapWithSurface(char** ppBits,DWORD* pdwWidth,DWORD* pdwHeight,DWORD* pdwBytesPerPixel,IDirect3DSurface9* pSurface)
{
	// 565포맷만 됨...압축텍스쳐는 됨...
	...
	if (IsCompressed(ddsc.Format))
	{
		m_pD3DDevice->CreateOffscreenPlainSurface(ddsc.Width,ddsc.Height,D3DFMT_R5G6B5,D3DPOOL_SCRATCH,&pSurfaceTemp,NULL);
	...
	else
	{
		...
		// 압축 텍스쳐가 아니면..
		m_pD3DDevice->CreateOffscreenPlainSurface(ddsc.Width,ddsc.Height,ddsc.Format,D3DPOOL_SCRATCH,&pSurfaceTemp,NULL);
	}
	*pdwWidth = ddsc.Width;
...
BOOL CD3DResourceManager::DuplicateTexture(LPDIRECT3DTEXTURE9* ppTexDest,D3DPOOL pool,LPDIRECT3DTEXTURE9 pTexSrc,DWORD Flag)
{
	...
	for (i=0; i<miplevelnum; i++)
	{
		...
		if (D3D_OK != m_pD3DDevice->GetRenderTargetData(pSrc, pDest))
		{
			OutputDebugString("GetRenderTargetData Failed!!");
		}
...
BOOL CD3DResourceManager::CopyTexture(LPDIRECT3DTEXTURE9 pTexDest,LPDIRECT3DTEXTURE9 pTexSrc)
{
	IDirect3DSurface9*		pSrc;
	IDirect3DSurface9*		pDest;
...
BOOL CD3DResourceManager::SetTextureBorder(LPDIRECT3DTEXTURE9 pTex,DWORD dwBorderColor)
{
	BOOL			bResult = FALSE;
	HRESULT			hr;
	...
	D3DSURFACE_DESC		ddsc;
	IDirect3DTexture9*	pTexTemp = NULL;
