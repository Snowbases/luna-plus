...
//		key.dwObjCount = m_dwObjCount;
		...
		if (bNeedUpdate)
		{
			...
			if(D3D_OK != pRenderVB->LockVertexPtr(&pVertex, 0))
			{
				goto lb_return;
			}
...
lb_render:
	...
	for (i=0; i<2; i++)
	{
		pDevice->SetSamplerState( i, D3DSAMP_ADDRESSU,D3DTADDRESS_WRAP);
		pDevice->SetSamplerState( i,D3DSAMP_ADDRESSV,D3DTADDRESS_WRAP);
	}
	...
	pDevice->SetVertexShaderConstantF(12, (float*)&v4DirLight,  1 );	// light direction

	
	MATRIX4	matTemp;
	matTemp = *pResult;
	TransposeMatrix(&matTemp);
	pDevice->SetVertexShaderConstantF(0, (float*)&matTemp, 4);	
	...
	TransposeMatrix(&matTemp);
	pDevice->SetVertexShaderConstantF(8, (float*)&matTemp, 4);	
	...
	pDevice->SetVertexShaderConstantF(33, fHalf, 1);
...
//	dwAlpha = 125;
	...
	pDevice->SetVertexShaderConstantF(0, fAlpha, 2);


	CAMERA_DESC* pCameraDesc;
	pCameraDesc = m_pRenderer->INL_GetCameraDesc();
	pDevice->SetVertexShaderConstantF(24, (float*)&pCameraDesc->v3EyeDir, 1);		

	pDevice->SetStreamSource( 0,pRenderVB->GetVertexBuffer(),0,pRenderVB->GetVertexSize());

	pDevice->SetVertexShader(NULL);
	//pDevice->SetFVF(m_vsHandle.dwShader);
	pDevice->SetFVF(pRenderVB->GetVertexShader());
	...
	pDevice->SetVertexShaderConstantF(25, (float*)&matTex, 4);	
...
//			pDevice->SetPixelShader(m_psDot3ReflectHandle.dwShader);
		...
		pDevice->SetIndices(m_pFaceGroup[i].GetIndexBuffer());
		pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,pRenderVB->GetStartIndex(),0,m_dwVertexNum,m_pFaceGroup[i].GetIBStartIndex(),m_pFaceGroup[i].GetFacesNum());
...
//	pDevice->SetRenderState(D3DRS_ZWRITEENABLE,FALSE);
	...
	{
		pDevice->SetIndices(m_pFaceGroup[i].GetIndexBuffer());
		pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,pVB->GetStartIndex(),0,m_dwVertexNum,m_pFaceGroup[i].GetIBStartIndex(),m_pFaceGroup[i].GetFacesNum());
	}
//	pDevice->SetRenderState(D3DRS_ZWRITEENABLE,TRUE);
