...
BOOL CVertexBuffer::SetVertexPosList(VECTOR3* pv3,DWORD dwNum)
{
	...
	if (!m_pVB)
		goto lb_return;
	else if(D3D_OK != m_pVB->Lock(m_dwStartIndex*m_dwSize,m_dwSize*dwNum, (void**)&pDest, 0 ))
		goto lb_return;
...
BOOL CVertexBuffer::SetVertexPosList(DWORD dwDestPitch,DWORD dwSrcPosX,DWORD dwSrcPosY,HFIELD_DESC* pHFDesc,DWORD dwSampleStep)
{
	...
	if (!m_pVB)
		goto lb_return;
	else if(D3D_OK != m_pVB->Lock(m_dwStartIndex*m_dwSize,m_dwSize*m_dwVerticesNum, (void**)&pDest, 0 ))
		goto lb_return;
...
{
	return m_pVB->Lock(m_dwStartIndex*m_dwSize,m_dwSize*m_dwVerticesNum, (void**)ppVPtr,dwFlag);
...
BOOL CVertexBuffer::SetTexCoord(TVERTEX* ptv,TEX_COORD_OFFSET offset,DWORD dwNum)
{
	...
	m_pVB->Lock(m_dwStartIndex*m_dwSize,m_dwSize*dwNum, (void**)&pDest, 0 );
