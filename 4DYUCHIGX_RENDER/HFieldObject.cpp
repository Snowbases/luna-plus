...
lb_alpha_tex_check:
			IDirect3DTexture9*	pAlphaTex = (IDirect3DTexture9*)pTileBufferDesc->alphaMapDesc.pVoidExt;
			if (!pAlphaTex)
			{
		...
		pDevice->SetIndices(m_IB.GetIndexBuffer());
...
BOOL CHFieldObject::UpdateAlphaMap(TILE_BUFFER_DESC* pTileBufferDesc)
{
	...
	{
		IDirect3DTexture9*	pAlphaTexture = (IDirect3DTexture9*)pAlphaMapDesc->pVoidExt;
		if (!pAlphaTexture)
		{
...
BOOL __stdcall CHFieldObject::ReBuildMesh(DWORD dwDestPitch,HFIELD_DESC* pHFDesc , DWORD* pdwColor)
{
	...
	if(D3D_OK != m_VB.LockVertexPtr(&pDest,0))
	{
		return FALSE;
	}
...
void __stdcall CHFieldObject::CleanupAlphaMap()
{
	...
	for (i=0; i<m_pControllerStatic->GetTileBufferDescNum(); i++)
	{
		...
		IDirect3DTexture9*	pAlphaTexture = (IDirect3DTexture9*)pTileBufferDesc->alphaMapDesc.pVoidExt;
