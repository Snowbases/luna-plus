...
//	LPDIRECT3DSURFACE9			pDefaultZBuffer;
	...
	HRESULT rt = m_pD3DDevice->TestCooperativeLevel();    // 상태 조사
	if (bTest)
	{
		while( rt != D3DERR_DEVICENOTRESET )
		{
			Sleep( 30 );									// 30 정도.Ofon
			rt = m_pD3DDevice->TestCooperativeLevel();    // 상태를 다시 조사
		}
...
BOOL __stdcall CoD3DDevice::CaptureScreen(char* szFileName)
{
	...
	// CopyRect(DX8) 대신에 UpdateSurface()를 쓰지만, 이 경우는 랜더타겟에서 복사해야하기 때문에 사용하는 함수
