...
CBaseObject* CScene::AllocObject(OBJECT_TYPE type,char* szObjName)
{
	...
	if (pModel)
	{
		...
		{
			MessageBox(NULL,"루트 모델과 서브 모델의 오브젝트 이름이 일치하지 않습니다.","Error",MB_OK);
			m_bSuccess = FALSE;
		}
...
CBaseObject* CScene::InsertObject(INode* pNode)
{
	...
	ObjectState os = pNode->EvalWorldState(0);	// 현재프레임, 0으로 고정한다.
...
//	mesh->buildNormals();
	...
	if (dwVertexNum)
	{
		...
		pMesh->CreateNormalVertexList(NULL);	// 실제로 사용하진 않지만 익스포트의 일관성을 위해 만들어만 놓는다.
...
//		pv3Map = new VECTOR3[dwFaceNum*3];
	...
	//////////////////////////////////////////////////////////////////////////////////////////////
	// uv가 겹치면 버텍스를 쪼개므로 노멀을 미리 만들어두자..
...
*/
	...
	if (dwTexVertexNum)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////
		//
		// 텍스쳐가 있을 경우 tvertex에 맞춰서 버텍스 다시 생성
		//
		///////////////////////////////////////////////////////////////////////////////////////////////
	...
	for (i=0; i<MAXEXP_MAX_MATERIAL_NUM; i++)
	{
		if (dwMtlIndex[i])
		{
			...
			{
				wsprintf(txt,"매터리얼이 설정되어있지 않은 면이 있습니다.:%s.문제의소지가있습니다.",pObj->GetObjectName());
	...
	// 버텍스 카피 
	// 이 버텍스는 위의 과정에서 tvertex를 참고, 재생성된 것이어야한다.
	if (pv3)
	{
		pMesh->CreateVertexList(dwVertexNum);
		for (i=0; i<dwVertexNum; i++)
...
DWORD CScene::AddMaterial(Mtl* pMtl,DWORD dwSubMtlIndex)
{
	...
	pMtl = pPrv->GetSubMtl(dwSubMtlIndex);
...
lb_set_mtl:
	...
	// 2005.1.6.일 수정.매터리얼 인덱스가 1부터 나오는 것을 0부터 나오도록..
...
BOOL CScene::WriteFile(TCHAR* uszFileName)
{
	...
	while (pCurObj)
	{
		...
		fwrite(&type,sizeof(DWORD),1,fp);
		entry = ftell(fp);					// 길이를 기록하기 위한 파일 옵셋 
		fseek(fp,4,SEEK_CUR);				// 4바이트 건너뛰고
	
		size = pCurObj->WriteFile(fp);		// WRITE 한 사이즈
		
		fseek(fp,entry,SEEK_SET);			// 길이를 기록
		fwrite(&size,sizeof(DWORD),1,fp);
		fseek(fp,size,SEEK_CUR);			// 다음 청크를 기록할 옵셋으로..
...
BOOL CScene::SetHirerarchy()
{
	...
	if (pModel)
	{
		...
		{
			MessageBox(NULL,"루트모델과 서브모델의 오브젝트 개수가 일치하지 않습니다.","Error",MB_OK);
			m_bSuccess = FALSE;
			return FALSE;
		}
	}
