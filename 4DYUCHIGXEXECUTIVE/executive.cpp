...
BOOL __stdcall CoExecutive::InitializeFileStorageWithoutRegistry(char* szFileName,DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum,BOOL bFileShareMode)
{
...
lb_fail:
	...
	bResult = InitializeFileStorageCommon(dwMaxFileNum,dwMaxFileHandleNumAtSameTime,dwMaxFileNameLen,accessMethod,pPackFileList,dwNum, bFileShareMode);

lb_return:
	return bResult;

}
BOOL __stdcall CoExecutive::InitializeFileStorage(DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum,BOOL bFileShareMode)
{
	BOOL	bResult = FALSE;
...
BOOL CoExecutive::InitializeFileStorageCommon(DWORD dwMaxFileNum,DWORD dwMaxFileHandleNumAtSameTime,DWORD dwMaxFileNameLen,FILE_ACCESS_METHOD accessMethod,PACKFILE_NAME_TABLE* pPackFileList,DWORD dwNum, BOOL bFileShareMode)
{
	BOOL	bResult = FALSE;
	...
	for (i=0; i<dwNum; i++)
	{
		// 090831 ShinJS --- Pack파일 Create/Load 시 Share Mode 설정 추가하여 멀티 클라이언트 가능토록 함.
		m_pPackFileHandleList[m_dwPackFilesNum] = m_pFileStorage->MapPackFile(pPackFileList[i].szFileName, bFileShareMode);
		m_dwPackFilesNum++;
	}
	bResult = TRUE;
lb_return:
...
// 090422 ShinJS --- GXObject의 ParentMatrix를 구한다, ParentMatrix의 위치값은 Attach된 좌표
